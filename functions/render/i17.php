...
function render_tabla_i17 () {
	...
	if (cG('niveles')) {
		$nivel = cG('niveles');
		if ($nivel == 'anexo') {
			$gastos_generales = get_db_data(array('listar-gastos-mediosm-i17-2-filtro-anexo', $custom_query));		
		} else if ($nivel == 'contrato') {
			$gastos_generales = get_db_data(array('listar-gastos-mediosm-i17-2-filtro-contrato', $custom_query));
		} else if ($nivel == 'proyecto') {
			$gastos_generales = get_db_data(array('listar-gastos-mediosm-i17-2-filtro-proyecto', $custom_query));
		} else {
			$gastos_generales = get_db_data(array('listar-gastos-mediosm-i17-2','WHERE Ejercicio=' . date('Y')));
		}
	}
	...
	if (cG('excel')=='true') {
		...
		$csv .= 'Codigo' . $separator.'Descripcion' . $separator . 'Enero' . $separator . 'Febrero' . $separator . 'Marzo' . $separator . 'Abril' . $separator . 'Mayo' . $separator 
		. 'Junio' . $separator . 'Julio' . $separator . 'Agosto' . $separator . 'Septiembre' . $separator . 'Octubre' . $separator . 'Noviembre' . $separator . 'Diciembre' . $separator
		. 'Total anual' . "\n";
...
function render_tabla_facturacion_mes_i17 () {
	if (cG('ejercicios')) {
		$ejercicio = cG('ejercicios');
		if ($ejercicio != 0) {
			$custom_query = 'WHERE Ejercicio=' . $ejercicio;
...
function render_tabla_gastos_mes_i17 () {
	if (cG('ejercicios')) {
		$ejercicio = cG('ejercicios');
		if ($ejercicio != 0) {
			$custom_query = 'WHERE Ejercicio=' . $ejercicio;
