...
function render_tabla_i8 () {
	...
	*/
	...
	if (isset($sufac)) {
		if ($sufac!='')	{
			$where_query.="AND factura LIKE '%".$sufac."%' ";
			$default_query=false;
		}
	}
	if (isset($impf)) {
	...
	if($default_query) {
		...
		$query_to_show='';
		$data=get_db_data(array('listado-completo-filtrado-i8', $where_query));
	}
	...
	if ($data===false) {
		...
		if (cG('excel')=='true') {
			...
			foreach ($data as $row) {
				...
				$csv.=
					'"'		.trim(	$row['codigocliente']						)		.'"'.$separator.''.
					'"'		.trim(	$row['razonsocial']							)		.'"'.$separator.''.
					'"'		.trim(	$row['factura']								)		.'"'.$separator.''.
			...
			foreach ($data as $row) {
				...
				if ($codigoActual != $row['codigocliente']) {
					...
					$sumaImporte = 0;
					$codigoActual = $row['codigocliente'];
				...
				<tr '.$style.'>
					...
					<td>'.utf8_encode($row['contrato']).'</td>
...
function make_filter_8 () {
	
	$filtro = file_get_contents_utf8(ABSPATH.'/template/filtro-8.html');
