...
class facturacionMensual extends bd {
    ...
    public function pintarTabla ($filtros, $excel = false) {
    	...
    	$group = 'GROUP BY Ejercicio, Mes, TipoDocum, CodigoProyecto, DescripcionProyecto, Proyecto, Descripcion, FacturadoMes, ImporteContrato, FacturadoOrigen, CarteraPendiente, TotalProyectoFacturadoMes, TotalImporteContratos, TotalProyectoFacturadoOrigen, TotalProyectoCarteraPendiente, EstadoFacturacion, ObservacionesProyecto, Activa';
		...
		if ($excel) {
    		...
    		if (is_array($aTabla)) {
				...
				foreach ($aTabla as $a) {
					...
					$html .= 
							'<tr style="background-color:' . $colorFila . ';' . $bordeFinProyecto 
								. '" ejercicio="' . $a['Ejercicio'] . '" mes="' . $a['Mes'] . '" codigoProyecto="' . $a['CodigoProyecto'] . '" anexo="' . $a['Proyecto'] . '">
		...
		return json_encode(array('html' => $html, 'empresa' => $this->empresa, 'consulta' => 'select ' . $select . ' from ' . $from . ' ' . $where . $order));
    }
